...
@test "setup: pond is not loaded" (not contains $pond_home/$pond_name $fish_function_path) $status -eq $success
@test "pond init: success exit code" (__pond_init) $status -eq $success
@test "pond init: autoloaded init function inaccessible" (functions -q {$pond_name}_{$pond_init_suffix}) $status -eq $fail
@test "pond init: autoloaded deinit function inaccessible" (functions -q {$pond_name}_{$pond_deinit_suffix}) $status -eq $fail
__pond_tear_down
...
@test "setup: $pond_name_prefix-3 pond is not loaded" (not contains $pond_home/$pond_name_prefix-3 $fish_function_path) $status -eq $success
@test "pond init: success exit code" (__pond_init) $status -eq $success
@test "pond init: $pond_name_prefix-1 autoloaded init function inaccessible" (functions -q {$pond_name_prefix-1}_{$pond_init_suffix}) $status -eq $fail
@test "pond init: $pond_name_prefix-2 autoloaded init function inaccessible" (functions -q {$pond_name_prefix-2}_{$pond_init_suffix}) $status -eq $fail
@test "pond init: $pond_name_prefix-3 autoloaded init function inaccessible" (functions -q {$pond_name_prefix-3}_{$pond_init_suffix}) $status -eq $fail
@test "pond init: $pond_name_prefix-1 autoloaded deinit function inaccessible" (functions -q {$pond_name_prefix-1}_{$pond_deinit_suffix}) $status -eq $fail
@test "pond init: $pond_name_prefix-2 autoloaded deinit function inaccessible" (functions -q {$pond_name_prefix-2}_{$pond_deinit_suffix}) $status -eq $fail
@test "pond init: $pond_name_prefix-3 autoloaded deinit function inaccessible" (functions -q {$pond_name_prefix-3}_{$pond_deinit_suffix}) $status -eq $fail
__pond_tear_down
